  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-b from-black/50 to-transparent backdrop-blur-sm">
      <div className="flex items-center justify-between px-6 py-2 max-w-7xl mx-auto">
          ...
          >
            {user ? (
              <>
                ...
                <span className="text-sm hidden sm:block text-white">{t.profile}</span>
              </>
            ) : (
              <>
                <User className="w-5 h-5" />
                <span className="text-sm hidden sm:block text-white">{t.signIn}</span>
              </>
            )}
          </div>
        </div>
        ...
        <div className={`flex items-center ${searchBgClass} backdrop-blur-sm rounded-full px-4 py-1.5 cursor-pointer transition-all duration-300 min-w-80`} onClick={onSearchClick}>
        ...
        {/* Right Section - WikTok + Today + Discover */}
        <div className="flex items-center gap-4">
          <Compass 
          ...
          <div className="text-xl font-bold text-wikitok-red cursor-pointer" onClick={handleWikTokClick}>
            WikTok
          </div>
        </div>
      </div>
