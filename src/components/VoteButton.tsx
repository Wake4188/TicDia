...
const VoteButton = ({ articleId, articleTitle, articleUrl }: VoteButtonProps) => {
  const { user } = useAuth();
  ...
  return (
    <div className="flex flex-col items-center text-white">
      <button 
        className={`sidebar-icon ${isVoted ? 'text-tictok-red' : ''} ${isLoading ? 'opacity-50' : ''}`}
        onClick={handleVote}
        disabled={isLoading}
      >
        <ArrowUp className={`w-6 h-6 ${isVoted ? 'fill-current' : ''}`}/>
      </button>
      <span className="text-xs font-medium">{isVoted ? t.upvote + "d" : t.upvote}</span>
