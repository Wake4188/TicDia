...
-- Add constraints for data validation and security
ALTER TABLE public.saved_articles 
  ADD CONSTRAINT IF NOT EXISTS check_article_title_length 
  CHECK (LENGTH(article_title) <= 500);

ALTER TABLE public.saved_articles 
  ADD CONSTRAINT IF NOT EXISTS check_article_url_format 
  CHECK (article_url IS NULL OR article_url ~* '^https?://');

ALTER TABLE public.user_preferences 
  ADD CONSTRAINT IF NOT EXISTS check_background_opacity_range 
  CHECK (background_opacity BETWEEN 0 AND 100);

ALTER TABLE public.user_preferences 
  ADD CONSTRAINT IF NOT EXISTS check_highlight_color_format 
  CHECK (highlight_color ~* '^#[0-9a-f]{6}$');
